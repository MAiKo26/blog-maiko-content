...
So next we have Kiro from Amazon and Antigravity from Google and neither offer anything special (haven't tried Microsoft's Copilot much) and same goes for Kilo/MimoCode which's are OpenCode forks.

So at the end the current best Harnesses in my opinion are Cursor, Codex and OpenCode; Cursor especially ever since AI Coding took off it hasn't lost the number one spot in my opinion as it consistently been the best Harness to use wheter in CLI, Cloud, Agents interface or VSCode Fork Interface.
...
I think all of them have great usage and depending on the latest release of these labs they probably take the lead but the ones I setelled on are Composer 2.5, Mimo V2.5 (not pro) and Deepseek V4 Flash.
